...
class OSCAgent(Server):
	...
	def __init__(self, inport = 4567, outport = 1234, sequence = [], labels = [], model_type = "FactorOracleNavigator", equiv = (lambda x,y : x == y),
		label_type = None, content_type = None, authorized_transformations = [0], continuity_with_future = [0.0, 1.0], max_length_osc_mess = 100, *args, **kwargs):
		Server.__init__(self, inport = inport, outport = outport, *args, **kwargs)
		
		self.generation_handler = GenerationHandler(sequence, labels, model_type, equiv, label_type, content_type, authorized_transformations, continuity_with_future)
	...
	def handle_new_query(self, *args):
		...
		new_query = Query(start_date = start_date, start_unit = start_unit, start_type = start_type, scope_duration = scope_duration, scope_unit = scope_unit, handle = handle, label_type = label_type)

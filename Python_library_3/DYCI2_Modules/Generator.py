...
from DYCI2_Modules.Query import *
...
class GenerationHandler(Generator):
	...
	#TODO : not used yet, the queries are directly processed
	#TODO : work on "merge" and "reorder"
	#TODO : crucial part for a real merge improtek+somax ?
	def receive_query(self,query,print_info = False):
		"""
		...
		**TODO: for the moment only "append" and immediate processing.**
		...
		# IF QUERY concerns current time --> PROCESS immediately
		if query.scope["unit"] == "event":
	...
	# TODO: priority between queries expressed in events and in ms
		...
		if self.initial_query or (self.current_performance_time["event"] >= 0 and self.current_performance_time["ms"] >= 0) or query.start["type"] == "absolute":
			...
			if query.start["unit"] == "event":
				...
				if index_for_generation > 0 and index_for_generation < self.current_generation_time["event"]:
					#print("PROCESS 0.0")
					print("""USING EXECUTION TRACE : index_for_generation = {} / self.current_generation_time["event"] = {}""".format(index_for_generation , self.current_generation_time["event"]))

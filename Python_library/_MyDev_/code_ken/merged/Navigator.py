...
class Navigator(object):
	...
	def __setattr__(self, name_attr, val_attr):
		...
		if name_attr == "current_position_in_sequence" and val_attr > -1:

			self.current_navigation_index += 1
			print("-----------------\nCURRENT POS --> NAVIGATION INDEX: {}".format(self.current_navigation_index))
			print("NEW POSITION IN SEQUENCE: {}".format(val_attr))
			print("LEN EXECUTION TRACE: {}".format(len(self.execution_trace)))
			print("self.current_navigation_index")
			print(self.current_navigation_index)
			print(self.execution_trace.keys())

			if self.current_navigation_index > 0 and val_attr == self.execution_trace[self.current_navigation_index-1]["current_position_in_sequence"] + 1:
	...
	def __init__(self, sequence = [], labels = [], max_continuity = 20, control_parameters = [], execution_trace_parameters = [], equiv = (lambda x,y : x == y)):
	...
	def update_history_and_taboos(self, index_in_sequence):
		...
		if self.current_navigation_index > 0:
			...
			previous_position_in_sequence = self.execution_trace[self.current_navigation_index-1]["current_position_in_sequence"]
			#print("Previous position in sequence = {}".format(previous_position_in_sequence))
			if self.current_navigation_index > 1:
				previous_previous_continuity_in_sequence = self.execution_trace[self.current_navigation_index-2]["current_continuity"]
	...
	def free_navigation(self, length, new_max_continuity = None, forward_context_length_min = 0, init = False, equiv = None, print_info = False):
		...
		if init:
			self.reinit_navigation_param()
			authorized_indexes = self.filter_using_history_and_taboos(range(1,self.index_last_state()))
	...
	def filter_using_history_and_taboos(self,list_of_indexes):
		filtered_list = [i for i in list_of_indexes if (not (self.history_and_taboos[i] is None) and (self.avoid_repetitions_mode < 2 or self.avoid_repetitions_mode >= 2 and self.history_and_taboos[i] == 0))]
	...
	def find_matching_label_without_continuation(self, required_label, authorized_indexes, equiv = None):
		...
		if self.no_empty_event:
			possible_states = [p for p in range(1,self.index_last_state()+1) if equiv(self.labels[p],required_label) and p in authorized_indexes]
	...
	def find_prefix_matching_with_labels(self, use_intervals, labels, list_of_labels, continuity_with_future, authorized_indexes, authorized_transformations, sequence_to_interval_fun, equiv_interval, equiv, ):
		...
		if use_intervals:
			index_delta_prefixes, max_length = filtered_prefix_indexing_intervals(sequence = labels, pattern = list_of_labels, 

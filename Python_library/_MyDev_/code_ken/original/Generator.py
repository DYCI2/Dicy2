...
class Generator (object):
	...
	**Nika, Chemillier, Assayag, "ImproteK: introducing scenarios into human-computer music improvisation", 
	...
	def __init__(self, sequence = [], labels = [], model_navigator = "FactorOracleNavigator", equiv = (lambda x,y : x == y), 
		...
		try:
			...
			try:
				...
				print("Unknown model navigator. \n Available classes: {}".format(implemented_model_navigator_classes.keys()), exception)
				return None
			else:
				self.model_navigator = model_navigator
				self.memory = implemented_model_navigator_classes[self.model_navigator](sequence = sequence, labels = labels, label_type = label_type, content_type = content_type, equiv = equiv)
	...
	def use_intervals(self):
		...
		return (not (self.memory.label_type is None)) and self.memory.label_type.use_intervals and len(self.authorized_tranformations) > 0 and self.authorized_tranformations!= [0]
	...
	def formatted_output_couple_content_transfo(self):
		...
		for i in range(0,len(self.current_generation_output)):
			...
			if self.current_generation_output[i]:
				if self.transfo_current_generation_output[i] and type(self.transfo_current_generation_output[i]) == TransposeTransform:
					result.append([self.current_generation_output[i],self.transfo_current_generation_output[i].semitone])
				else:
	...
	def generation_matching_query(self,query,print_info = False):
		...
		"""
		print("GENERATION MATCHING QUERY 0")
		output = None
		self.transfo_current_generation_output = []
		...
		if query.scope["unit"] == "event":
			if query.scope["duration"] == 1:
				if query.handle[0] is None:
					...
					print(output)
					self.transfo_current_generation_output = [self.current_transformation_memory]*len(output)
					print("... GENERATION MATCHING QUERY 1 LABEL OK")
	...
	def handle_scenario_based_generation_one_phase(self, list_of_labels, print_info = False, shift_index = 0):
		...
		if self.use_intervals():
			index_delta_prefixes, max_length = filtered_prefix_indexing_intervals(sequence = self.memory.labels, pattern = list_of_labels, 
				...
				sequence_to_interval_fun = self.memory.label_type.make_sequence_of_intervals_from_sequence_of_labels, 
				equiv = self.memory.label_type.equiv_mod_interval, 
				print_info = False)
			# index_delta_prefixes, max_length = filtered_prefix_indexing_intervals(sequence = self.memory.labels, pattern = list_of_labels, 
			#  length_interval = self.continuity_with_future, authorized_indexes = authorized_indexes,
		...
		if not s is None:
			print("\n\n-->HANDLE SCENARIO BASED ONE PHASE SETS POSITION: {}<--".format(s))
			print("---------------------self.memory.current_position_in_sequence FROM TO-----------")
			...
			if t != 0:
				...
				if print_info:
					print("{} NEW STARTING POINT {} (orig. {}): {}\nlength future = {} - FROM NOW {}".format(shift_index, self.current_transformation_memory.encode(self.memory.labels[s]), self.memory.labels[s], self.memory.current_position_in_sequence, length_selected_prefix, self.current_transformation_memory ))
			else:
				s_content = self.memory.sequence[s]
				print(s_content)
				if print_info:
					print("{} NEW STARTING POINT {}: {}\nlength future = {} - FROM NOW No transformation of the memory".format(shift_index, self.memory.labels[s], self.memory.current_position_in_sequence, length_selected_prefix))
...
class GenerationHandler(Generator):
	...
	"""
		...
		#print(self.memory.current_position_in_sequence)
		print("""self.current_performance_time["event"]:""")
		print(self.current_performance_time["event"])
		print("""self.current_generation_time["event"]:""")
		print(self.current_generation_time["event"])
		print("""QUERY""")
		print(query)
		...
		if self.initial_query or (self.current_performance_time["event"] >= 0 and self.current_performance_time["ms"] >= 0) or query.start["type"] == "absolute":
			print("THE QUERRY IS PROCESSED")
			...
			if query.start["unit"] == "event":
				...
				print("PROCESS 0")
				print("index_for_generation:")
				print(index_for_generation)
				if index_for_generation > 0 and index_for_generation < self.current_generation_time["event"]:
					print("PROCESS 0.0")
					print("""index_for_generation = {} / self.current_generation_time["event"] = {}""".format(index_for_generation , self.current_generation_time["event"]))
			...
			print("""OLD OLD OLD self.current_generation_time["event"]""")
			print(self.current_generation_time["event"])
			...
			print("""NEW NEW NEW self.current_generation_time["event"]""")
			print(self.current_generation_time["event"])

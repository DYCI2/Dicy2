...
class Label(object):
	...
	def make_sequence_of_intervals_from_list(cls, init_list,  *args, **kwargs):
		return cls.make_sequence_of_intervals_from_sequence_of_labels(cls.make_sequence_of_labels_from_list(cls, init_list,  *args, **kwargs))
...
# CI DESSOUS CLASSES NON PRESENTES DANS EVENTS.PY DE SOMAX2
class ChordLabel(Label):
	...
	def __init__(self, label=None, first_chord_label = None, previous_chord_label = None):
		...
		if not previous_chord_label is None and issubclass(type(previous_chord_label), ChordLabel):
			self.interval_within_sequence["previous_chord_label"] = (previous_chord_label.delta_root(self),previous_chord_label)
		else:
	...
	def get_label(self, ctype="chord"):
		if ctype=='id':
			...
			return copy(self.root), copry(self.chordtype)

	# NONE, TUPLE,.... SHOULD BE CONVERTED INTO LABEL !!!! JOAKIM
	def __eq__(self, a):
		if type(a)==type(None):
			return False
	...
	@classmethod
	def make_sequence_of_labels_from_list(cls, list_of_labels, init_first_chord_label = None, init_previous_chord_label = None, **kwargs):
# METTRE DANS LES RECOMMANDATIONS DE RAPPELER PREMIER ET PRECEDENT ? OU BIEN DANS learn_event ligne environ 85 de generator.py
	...
	def transpose_root(self, i):
		self.normalize_root()
		l = ["c", "c#", "d", "eb", "e", "f", "f#", "g", "g#", "a", "bb", "b", "c", "c#", "d", "eb", "e", "f", "f#", "g", "g#", "a", "bb", "b"]
	...
	@classmethod
	def make_sequence_of_intervals_from_list(cls, init_list, init_first_chord_label = None, init_previous_chord_label = None):
		return chord_labels_sequence_to_interval(make_sequence_of_chord_labels(init_list, init_first_chord_label, init_previous_chord_label))

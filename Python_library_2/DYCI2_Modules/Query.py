...
#TODO : add inputs about "label_type" in the doc
#TODO : define query algebra
...
class Query:
	...
	def __init__(self, start_date = 0, start_unit = "event", start_type = "absolute", handle = [None], label_type = None, scope_duration = 0, scope_unit = "event", behaviour = "merge"): 
		#TODO : EXCEPTIONS for str arguments
		self.label_type = label_type

		try:
			assert type(start_date)==int and (start_unit == "event" or start_unit == "ms") and (start_type == "absolute" or start_type == "relative")
		except AssertionError as exception:
			print(""" start_date = int \n start_unit = "event" | "ms" \n start_type = "absolute" | "relative" """, exception)
			return False
		else:
		...
		try:
			...
			print(""" scope_duration = int \n scope_unit = "event" | "ms" """, exception)
			return False
		else:	
	...
	def __repr__(self):
		return "Query:\nStart = {} ({},{})\nScope = {} ({})\nHandle = {}".format(self.start["date"], self.start["unit"], self.start["type"], self.scope["duration"], self.scope["unit"], self.handle)
	...
	def relative_to_absolute(self, current_performance_time_event = None, current_performance_time_ms = None):
		try:
			assert (not ((current_performance_time_event is None) and (current_performance_time_event is None))) and ((not (current_performance_time_event is None) and self.start["unit"] == "event") or ((not (current_performance_time_ms is None) and self.start["unit"] == "ms")))
		except AssertionError as exception:

...
list_of_labels_1 = ["d m7", "d m7", "g 7", "g 7", "c maj7","c maj7","c# maj7","c# maj7", "d# m7", "d# m7", "g# 7", "g# 7", "c# maj7", "c# maj7"]
...
for c in sequence:
	print("{}: Interval previous label = {} - Interval first label = {}".format(c.label, c.delta_root_previous_label_in_sequence(), c.delta_root_first_label_in_sequence()))
...
prefixes_delta, length_longest_prefix = filtered_prefix_indexing_intervals(sequence, pattern, 
	...
	equiv = ChordLabel.equiv_mod_interval, 
	print_info = 0)
print("\nFiltered prefixes MODULO TRANSPOSITION of \n{} \nin \n{}\n with authorized indexes = {}, authorized intervals of transposition = {}:".format(pattern, sequence, authorized_indexes, authorized_intervals))
for length,list_of_left_pos_and_deltas in prefixes_delta.items():
	print("Length {}: ".format(length))
	for p in list_of_left_pos_and_deltas:
		print("Left position {} with transposition = {}.".format(p[0],p[1]))
...
############################################################
# Multi-Level Label Tutorial
		
multi_label_1 = MultiLevelLabel(('a','b','c'))
print("\n{}".format(multi_label_1))
multi_label_2 = MultiLevelLabel(('d','b','z'))
multi_label_3 = MultiLevelLabel(('l','o','l'))

print("\n{} equiv {} ? --> {}".format(multi_label_1, multi_label_2, MultiLevelLabel.multi_level_equiv(multi_label_1, multi_label_2)))
print("{} equiv {} ? --> {}".format(multi_label_1, multi_label_3, MultiLevelLabel.multi_level_equiv(multi_label_1, multi_label_3)))

list_of_multi_labels_1 = [('t','vi','I'),('t','vi','VI7'),('t','v','II-'),('t','v','V7'),('s','i7','I7'),('s','i7','I7'),('s','iv','IV'),('s','iv','IV-')]
sequence = from_list_to_labels(labels = list_of_multi_labels_1, label_type = MultiLevelLabel)
list_of_multi_labels_2 = [('t','i','I'),('t','i','I'),('t','v','V7'),('t','v','V7')]
pattern = from_list_to_labels(labels = list_of_multi_labels_2, label_type = MultiLevelLabel)
print sequence
print pattern

prefixes_delta, length_longest_prefix = prefix_indexing(sequence, pattern, equiv = MultiLevelLabel.multi_level_equiv, print_info = 0)
print prefixes_delta
for length,list_of_left_pos_and_deltas in prefixes_delta.items():
	print("Length {}: ".format(length))
	for p in list_of_left_pos_and_deltas:
		print("Left position {}.".format(p))